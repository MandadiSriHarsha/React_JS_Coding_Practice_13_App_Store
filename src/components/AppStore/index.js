...
class AppStore extends Component {
  state = {category: tabsList[0].tabId, searchText: ''}

  displayAppsByCategory = selectedCategory => {
  ...
  render() {
    ...
    const newAppsList = appsList.filter(
      eachapp =>
        eachapp.category === category &&
        eachapp.appName.toLowerCase().includes(searchText),
    )
    return (
        ...
        <ul className="tabs-container">
          {tabsList.map(eachtab => (
            <TabItem
              key={eachtab.tabId}
              data={eachtab}
              filterApps={this.displayAppsByCategory}
              isSelected={eachtab.tabId === category}
            />
          ))}
        </ul>
        <ul className="apps-container">
          {newAppsList.map(eachapp => (
            <AppItem key={eachapp.appId} data={eachapp} />
          ))}
        </ul>
      </div>
    )
